...
from app.infra.retrieval.base import BaseRetrievalStore, BaseDocumentStore

import logging
logger = logging.getLogger("app.infra.retrieval.opensearch.store")
logger.info("Loading file...")

from opensearchpy import OpenSearch, helpers

class OpenSearchStore(BaseRetrievalStore, BaseDocumentStore):
    ...
    # ADD SINGLE DOCUMENT
    # -------------------------
    def add_document(self, chunk_id: str, content: str, embedding: list[float], metadata: dict | None = None):
    ...
    # BULK INGESTION
    # -------------------------
    def add_documents_bulk(self, chunks: list[dict]):
        ...
        for hit in response["hits"]["hits"]:
            ...
            results.append(
                RetrievedDocument(
                    ...
                    scores=ScoreBreakdown(dense_retrieval_score=hit["_score"])
                )
            )
        ...
        for hit in response["hits"]["hits"]:
            ...
            results.append(
                RetrievedDocument(
                    ...
                    scores=ScoreBreakdown(sparse_retrieval_score=hit["_score"])
                )
            )
